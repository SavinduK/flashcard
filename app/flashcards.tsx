...
const Flashcards = () => {
  ...
  const getRandomImage = async (folderPath: string): Promise<string> => {
    try {
      const items = new Directory(folderPath).list();
      const images = items.filter((item: Directory|File) => {
      ...
      const randomImage = images[randomIndex].uri
...
const styles = StyleSheet.create({
  container: {flex: 1,backgroundColor: "#deceac",},
  content: {flex: 1,alignItems: "center",justifyContent:'center'},
  title: { fontSize: 25, fontWeight: 'bold', marginBottom: 20, marginLeft: '5%', marginTop: '5%', color: '#555',textAlign:"center" },
  image: {width:'100%',height:'100%',borderRadius: 16,},
  card: {width: "100%",height: "100%",backgroundColor: "#deceac",borderRadius: 20,padding: 10,marginBottom:20,justifyContent: "center",alignItems: "center",},
});
